...
export function PrimaryButton({ children, onClick, icon: Icon, href, className = "" }) {
  ...
  if (href) return <Link href={href} className={cls} style={style} {...handlers}>{children}{Icon && <Icon size={16} />}</Link>;
  return <button type="button" onClick={onClick} className={cls} style={style} {...handlers}>{children}{Icon && <Icon size={16} />}</button>;
}

export function SecondaryButton({ children, onClick, icon: Icon, href, className = "" }) {
  ...
  if (href) return <a href={href} target={href.startsWith("http") ? "_blank" : undefined} rel="noreferrer" className={cls} style={style} {...handlers}>{children}{Icon && <Icon size={16} />}</a>;
  return <button type="button" onClick={onClick} className={cls} style={style} {...handlers}>{children}{Icon && <Icon size={16} />}</button>;
}
      ...
      <div className="max-w-4xl mx-auto px-6 text-center">
        <FadeIn>
          ...
          <div className="flex flex-wrap gap-4 justify-center mt-9">
            <PrimaryButton href="/quote" icon={ChevronRight}>Request a Quote</PrimaryButton>
            <SecondaryButton href={`https://wa.me/${whatsapp}`} icon={MessageCircle} className="!text-[#F7F4ED] !border-[#C8A542]">Chat on WhatsApp</SecondaryButton>
          </div>
        </FadeIn>
      </div>
    </section>
